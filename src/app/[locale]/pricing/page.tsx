  ...
  >
    <defs>
      <pattern id="islamicGrid" width="20" height="20" patternUnits="userSpaceOnUse">
        ...
        <circle cx="0" cy="0" r="1" strokeWidth="0.2" />
        <circle cx="20" cy="0" r="1" strokeWidth="0.2" />
        <circle cx="0" cy="20" r="1" strokeWidth="0.2" />
        <circle cx="20" cy="20" r="1" strokeWidth="0.2" />
      </pattern>
    </defs>
...
const IslamicStar = ({ className = "" }: { className?: string }) => (
  ...
  >
    ...
    >
      ...
      {/* Intricate inner alignments */}
      <rect x="60" y="60" width="80" height="80" rx="0.5" transform="rotate(15 100 100)" strokeWidth="0.5" strokeDasharray="2,2" />
      <rect x="60" y="60" width="80" height="80" rx="0.5" transform="rotate(30 100 100)" strokeWidth="0.5" strokeDasharray="2,2" />
      <rect x="60" y="60" width="80" height="80" rx="0.5" transform="rotate(60 100 100)" strokeWidth="0.5" strokeDasharray="2,2" />
      <rect x="60" y="60" width="80" height="80" rx="0.5" transform="rotate(75 100 100)" strokeWidth="0.5" strokeDasharray="2,2" />
...
export default function PricingPage() {
  ...
  // Dynamic commitment parameters to make weekly/monthly plans transparent
  const planDetails = {
    seeker: {
      hours: 2,
  ...
  const getPrice = (tierId: 'initiate' | 'scholar' | 'master') => {
    const annualPrice = planDetails[intensity][tierId].price;
  ...
  const faqs = [
    { 
      ...
      a: 'Absolutely. You can upgrade, downgrade, or adjust your weekly learning intensity from your student dashboard. Billing is adjusted pro-rata immediately.' 
    },
    { 
      q: 'What does the 1-on-1 mentorship entail?', 
      a: 'Mentorship sessions are scheduled with certified scholars. They provide direct Tajweed corrections, clarify complex fiqh concepts, check academic progress, and offer personalized spiritual guidance.' 
    },
    { 
      q: 'Is there a free trial for packages?', 
      a: 'Yes, we provide a 7-day free trial on all learning packages. You can explore the platform, attend live seminars, and book your initial assessment session without charges.' 
    },
    { 
      q: 'Are certificates or Ijazah awarded?', 
      a: 'Yes, students who successfully complete study modules and pass oral assessments on classical treatises are awarded certificates and authentic academic chains of transmission (Ijazah).' 
    },
    { 
      q: 'Do you offer financial aid or sponsorships?', 
      a: 'We ensure that financial hardship never prevents seekers from obtaining sacred knowledge. Zakat-backed scholarship options are available for students in need. Please apply via our support page.' 
    },
  ];
  ...
  return (
    <div className="bg-primary min-h-screen pt-12 relative overflow-hidden selection:bg-accent/30 selection:text-white">
      ...
      <div className="absolute top-1/3 right-1/4 w-[600px] h-[600px] bg-accent/5 rounded-full filter blur-[180px] pointer-events-none" />
      <div className="absolute bottom-10 left-1/3 w-[450px] h-[450px] bg-accent/5 rounded-full filter blur-[150px] pointer-events-none" />
      ...
      <section className="relative pt-16 pb-12 sm:pt-24 sm:pb-16 md:pt-28 overflow-hidden">
        <div className="max-w-5xl mx-auto text-center px-4 sm:px-6 lg:px-8 relative z-10">
          <AnimateIn direction="down" duration={0.8}>
            {/* Top Sub-Badge */}
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-accent/20 to-accent/5 border border-accent/30 backdrop-blur-md mb-6 shadow-[0_0_15px_rgba(212,175,55,0.1)]">
            ...
            {/* Display Title */}
            <h1 className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-7xl text-white mb-6 leading-tight drop-shadow-[0_4px_12px_rgba(0,0,0,0.5)]">
              {t('hero.title')}
            </h1>
            ...
            <div className="flex justify-center items-center gap-4 mb-8">
              ...
              <span className="material-symbols-outlined text-accent text-sm select-none">star</span>
              <div className="w-16 h-[1px] bg-gradient-to-r from-accent via-accent to-transparent" />
            </div>

            {/* Description Subtitle */}
            <p className="font-sans text-base sm:text-lg md:text-xl text-white/75 leading-relaxed max-w-3xl mx-auto px-4 drop-shadow-md">
              {t('hero.subtitle')}
            </p>
          </AnimateIn>
        </div>
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex items-center gap-4 p-1.5 bg-[#0D0D0D]/90 rounded-full border border-accent/15">
                  <button 
                    ...
                    className={`relative px-5 py-2 text-xs font-bold uppercase tracking-wider rounded-full transition-all duration-300 ${!isAnnual ? 'bg-gradient-to-r from-accent to-accent-hover text-white shadow-md shadow-accent/20' : 'text-white/50 hover:text-white'}`}
                  >
                    {t('toggle.monthly')}
                  </button>
                  <button 
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[9px] bg-white/20 px-1.5 py-0.5 rounded text-white font-bold">
                        -20%
                      </span>
                    </div>
                  </button>
              ...
              <div className="w-full flex flex-col items-center gap-3 border-t border-accent/10 pt-5">
                ...
                <div className="grid grid-cols-3 gap-2 w-full bg-[#0D0D0D]/80 p-1 rounded-xl border border-accent/10">
                  {(['seeker', 'dedicated', 'scholarly'] as const).map((mode) => {
                    const label = mode === 'seeker' ? 'Seeker (2h/wk)' : mode === 'dedicated' ? 'Dedicated (4h/wk)' : 'Scholarly (8h/wk)';
                    const isSelected = intensity === mode;
                    return (
                      <button
                        key={mode}
                        onClick={() => setIntensity(mode)}
                        className={`py-2 px-1 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-lg transition-all duration-300 cursor-pointer ${
                          isSelected 
                ...
                <p className="font-sans text-[11px] sm:text-xs text-white/50 text-center leading-relaxed max-w-md mt-1">
                  {intensity === 'seeker' && "Ideal for part-time seekers balancing study with basic professional/personal schedules."}
                  {intensity === 'dedicated' && "Balanced track offering robust knowledge progression and weekly personalized interactions."}
                  {intensity === 'scholarly' && "Immersive track designed for rapid advancement, deep text readings, and regular mentorship."}
                </p>
              </div>
      ...
      <section className="relative py-12 sm:py-16 md:py-20 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          {/* Main geometric background star */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] sm:w-[600px] sm:h-[600px] opacity-[0.02] sm:opacity-[0.03] z-0 pointer-events-none">
                  ...
                  <motion.div 
                    ...
                    className={`flex flex-col h-full rounded-3xl relative overflow-hidden transition-all duration-500 ${
                      isPopular 
                        ...
                        : 'border border-accent/15 shadow-lg bg-gradient-to-b from-[#121212]/85 to-[#0A0A0A]/95 hover:border-accent/35 hover:shadow-[0_0_30px_rgba(212,175,55,0.08)]'
                    }`}
                  >
                    ...
                    {/* Popular highlighted ribbon/badge */}
                    {isPopular && (
                    ...
                    {isPopular && (
                      <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-20">
                        <div className="bg-gradient-to-r from-accent via-[#B8922E] to-accent text-primary px-5 py-1.5 font-sans text-[10px] font-bold uppercase tracking-widest border border-accent/30 rounded-full shadow-[0_4px_15px_rgba(212,175,55,0.3)]">
                    ...
                    <div className="p-8 sm:p-10 flex flex-col flex-grow relative z-10">
                      ...
                      <div className="mb-6 pb-6 border-b border-accent/10">
                        <span className="font-sans text-[11px] font-bold uppercase tracking-[0.2em] text-accent block mb-2">
                          {tier.id === 'initiate' ? 'Foundational Track' : tier.id === 'scholar' ? 'Scholarly Track' : 'Mastery Track'}
                        </span>
                        ...
                        <div className="flex items-baseline gap-1 mb-3">
                          <span className="font-sans text-xl text-accent/80 font-bold self-start mt-1">$</span>
                          <span className="font-display text-5xl sm:text-6xl text-accent font-bold tracking-tight drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)]">
                            {price}
                          </span>
                          <span className="font-sans text-xs text-white/50 ml-1">
                            {isAnnual ? '/ yr' : '/ mo'}
                          </span>
                        </div>
                        ...
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-white/[0.03] border border-white/5 mb-4 text-white/70 text-xs">
                          ...
                          <span className="font-sans font-medium">{details.label}</span>
                          <span className="w-1 h-1 rounded-full bg-white/20" />
                          <span className="font-sans text-white/50 text-[10px]">{details.sessions}</span>
                        </div>
                      ...
                      <div className="flex-grow mb-8">
                        <p className="font-sans text-[11px] font-bold uppercase tracking-wider text-white/40 mb-4">
                          Included Features & Resources
                        </p>
                        <ul className="space-y-3.5">
                          {tier.features.map((feature) => (
                            <li key={feature} className="flex items-start gap-3 text-white/80 group">
                              <span className="material-symbols-outlined text-accent text-base flex-shrink-0 mt-0.5 select-none transition-transform duration-300 group-hover:scale-110">
                                check_circle
                              </span>
                              <span className="font-sans text-xs sm:text-sm text-white/75 group-hover:text-white transition-colors duration-200">
                                {t(`features.${feature}`)}
                              </span>
                            </li>
                          ))}
                      ...
                      <button 
                        onClick={() => {
                          // Dynamic redirection placeholder for checkout, retaining styling
                          window.location.href = `/${document.documentElement.lang || 'en'}/checkout?tier=${tier.id}&billing=${isAnnual ? 'annual' : 'monthly'}&intensity=${intensity}`;
                        }}
                        className={`w-full py-4 px-6 font-sans text-xs font-bold uppercase tracking-widest rounded-full transition-all duration-300 cursor-pointer shadow-md hover:-translate-y-0.5 ${
                          isPopular
                            ? 'bg-gradient-to-r from-accent to-[#B8922E] text-primary hover:shadow-accent/30 hover:shadow-[0_0_20px_rgba(212,175,55,0.25)] border border-accent/20'
          ...
          {/* Micro-notice under cards */}
          <div className="text-center mt-8 text-xs text-white/45 font-sans">
            * Registration fees support classical teachers and system maintenance. Scholarship applications processed within 3 business days.
          </div>

        </div>
      </section>

      {/* Features Comparison Grid Section */}
      <section className="relative py-16 sm:py-20 md:py-24 bg-gradient-to-b from-[#0D0D0D]/60 via-[#121212]/90 to-[#0D0D0D]/60 z-10 overflow-hidden">
        ...
        {/* Subtle geometric divider lines */}
        <div className="absolute inset-0 opacity-[0.02]"
          style={{
            backgroundImage: 'radial-gradient(circle at 10% 20%, #D4AF37 1px, transparent 1px)',
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <table className="min-w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-accent/15 bg-gradient-to-r from-accent/10 to-accent/2">
                  <th className="p-6 font-sans text-xs font-bold uppercase tracking-widest text-accent-hover">
              ...
              <tbody>
                {comparisonRows.map((row, idx) => (
                  <tr 
                    key={row.name} 
                    className={`border-b border-accent/5 transition-all duration-300 hover:bg-accent/3 ${
                      idx % 2 === 1 ? 'bg-white/[0.01]' : 'bg-transparent'
                    }`}
                  >
                    <td className="p-6 font-sans text-sm font-semibold text-white/80">
                      {t(`features.${row.name}`)}
                    </td>
                      ...
                      <td key={colIdx} className="p-6 text-center">
                        {val.includes('No') ? (
                          <div className="flex items-center justify-center">
                            <span className="material-symbols-outlined text-white/10 text-lg select-none">circle</span>
                          </div>
                        ) : (
                          <div className="flex flex-col items-center justify-center">
                            <span className="material-symbols-outlined text-accent text-lg select-none mb-0.5">check_circle</span>
                            <span className="font-sans text-[10px] sm:text-xs text-white/60 max-w-[150px] leading-tight">
                              {val}
                            </span>
                          </div>
                        )}
          ...
          <div className="grid grid-cols-1 gap-6 md:hidden">
            {comparisonRows.map((row) => (
              <div 
                key={row.name} 
                className="rounded-2xl border border-accent/10 bg-[#121212]/80 p-5 shadow-lg"
              >
                <h3 className="font-sans text-sm font-bold text-accent mb-4 border-b border-accent/10 pb-2">
                  {t(`features.${row.name}`)}
                </h3>
                  ...
                  {tiers.map((tier, colIdx) => {
                    ...
                    return (
                      <div key={tier.id} className="flex flex-col items-center gap-1.5 p-2 rounded-lg bg-white/[0.02] border border-white/5">
                        ...
                        {isNo ? (
                          <span className="material-symbols-outlined text-white/10 text-sm select-none">cancel</span>
                        ) : (
                          <>
                            <span className="material-symbols-outlined text-accent text-sm select-none">check_circle</span>
                            <span className="font-sans text-[9px] text-white/70 leading-tight">
                              {status}
                            </span>
                          </>
                        )}
          ...
          <div className="text-center mb-12 sm:mb-16">
            <AnimateIn>
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-accent/10 border border-accent/20 text-accent mb-4">
                <span className="material-symbols-outlined text-2xl select-none">help_outline</span>
              </div>
              <h2 className="font-display text-3xl sm:text-4xl md:text-5xl text-white mb-4 drop-shadow-lg">
                Frequently Asked Questions
              </h2>
              <p className="font-sans text-white/60 text-sm sm:text-base max-w-2xl mx-auto">
                Discover clarifications regarding our student programs, credentials, and custom scheduling options.
              </p>
            </AnimateIn>
          </div>
                ...
                <AnimateIn key={idx} delay={idx * 0.1}>
                  <div 
                    className={`rounded-2xl border transition-all duration-300 ${
                      isOpen 
                        ? 'border-accent/40 bg-[#161616]/80 shadow-[0_0_20px_rgba(212,175,55,0.05)]' 
                        : 'border-accent/15 bg-[#121212]/60 hover:border-accent/30 hover:bg-[#141414]/75'
                    }`}
                  >
                    <button
                    ...
                    >
                      <div className="flex items-center gap-3">
                        {/* Custom decorative dot */}
                        <span className="w-1.5 h-1.5 rounded-full bg-accent opacity-50 group-hover:opacity-100 transition-opacity" />
                        <h3 className="font-display text-base sm:text-lg text-white group-hover:text-accent transition-colors duration-200">
                          {faq.q}
                        </h3>
                      </div>
                      <motion.span
                        ...
                        className="material-symbols-outlined text-accent text-xl flex-shrink-0 ml-4 select-none"
                      >
                        expand_more
                      </motion.span>
                    </button>
      ...
      <section className="relative py-12 sm:py-16 md:py-20 bg-gradient-to-b from-[#0D0D0D]/60 via-[#101010]/85 to-[#0D0D0D]/60 z-10 overflow-hidden">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {[
              { icon: 'verified_user', title: 'Authentic Lineage (Sanad)', desc: 'We only present teachings verified by a direct chain of narration back to classical masters, maintaining integrity.' },
              { icon: 'swap_horiz', title: 'Fully Flexible Transfers', desc: 'Swap hours, freeze your packages, or adjust timings freely. We support modern life demands.' },
              { icon: 'support_agent', title: 'Dedicated Advisor Help', desc: 'Get assistance with custom curriculum selections, scheduling difficulties, or localized exams 24/7.' },
              { icon: 'paid', title: '30-Day Seekers Refund', desc: 'If your circumstances prevent study progression, request a refund on unused class sessions within 30 days.' },
            ].map((benefit, idx) => (
              <AnimateIn key={idx} delay={idx * 0.12}>
                <div className="group flex gap-5 p-7 rounded-2xl border border-accent/15 bg-gradient-to-br from-[#151515]/75 to-[#0F0F0F]/85 transition-all duration-300 hover:border-accent/40 hover:shadow-xl hover:-translate-y-1">
                  <div className="flex-shrink-0 w-11 h-11 bg-accent/10 border border-accent/25 rounded-xl flex items-center justify-center group-hover:bg-accent/20 transition-all duration-300">
                    <span className="material-symbols-outlined text-accent text-lg select-none">{benefit.icon}</span>
                  </div>
                  <div>
                    <h3 className="font-display text-base text-white mb-1.5 group-hover:text-accent transition-colors duration-200">
                      {benefit.title}
                    </h3>
      ...
      {/* Final CTA wrapped in Moorish arched containers */}
      <section className="relative py-16 sm:py-20 md:py-24 z-20">
        <div className="max-w-4xl mx-auto px-4 relative">
          <AnimateIn>
            ...
            >
              {/* Islamic motif watermark inside CTA */}
              <div className="absolute inset-0 opacity-[0.03] text-accent pointer-events-none">
              ...
              <div className="py-16 px-8 sm:px-12 md:px-16 text-center relative z-10 flex flex-col items-center">
                <h2 className="font-display text-3xl sm:text-4xl md:text-5xl text-white mb-6 leading-snug drop-shadow-md">
                ...
                <p className="font-sans text-white/70 text-sm sm:text-base max-w-xl mx-auto mb-10 leading-relaxed">
                  Join thousands of dedicated students studying Quran, Fiqh, Aqidah, and sacred Arabic online. Request your advisor review today.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 items-center justify-center w-full">
                  <Button 
                    ...
                    size="lg" 
                    onClick={() => {
                      window.location.href = `/${document.documentElement.lang || 'en'}/register?intensity=${intensity}`;
                    }}
                  ...
                  <Button 
                    ...
                    size="lg"
                    onClick={() => {
                      window.location.href = `/${document.documentElement.lang || 'en'}/contact`;
                    }}
